	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			if(toolResponse.get(0).equals("")) {
				menu.getViewInventoryButton().doClick();
				JOptionPane.showMessageDialog(null, "Item not found in inventory."
						+ "\nInventory has also been refreshed.","Error",JOptionPane.ERROR_MESSAGE);
			}
			else {
				String[] line = toolResponse.get(0).split("\t");
				
				int itemId = Integer.parseInt(line[0]);
				int newQuantity = Integer.parseInt(line[2]);
				int row, rowId;
				
				for(row = 0; row < menu.getTable().getRowCount(); row++) {
					rowId = Integer.parseInt((String)menu.getTable().getValueAt(row, 0));
					if(rowId == itemId)
						break;
				}
